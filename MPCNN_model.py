...
class TextMPCNN(object):
    ...
    def __init__(self, sequence_length, num_classes, filter_size, h_pool_type, p_pool_type,
                 h_ws_sizes, p_ws_sizes, l2_reg_lambda = 0):
        ...
        feah_holistic, feab_holistic = self.cal_holistic(m1_pool_holistic, m2_pool_holistic) # shape of [?, 900] and [?, 9]
        m1_pool_perspective = self.inference_perspective(s1)
        m2_pool_perspective = self.inference_perspective(s2) # shape of [2, ?, 300, 100]
        feah_perspective, feab_perspective = self.cal_perspective(m1_pool_perspective, m2_pool_perspective) # shape of [?, 600] and [?, 200]
        self.temp_concat = tf.concat([feah_holistic, feab_holistic, feah_perspective, feab_perspective], axis=1) # shape of [?, 1709]
        self.add_dropout()
        self.add_output()
        self.add_loss_acc()
    ...
    def inference_holistic(self, sent):
        m_pool = []
        for i, pool in enumerate(self.h_pool_type): # ['max', 'mean', 'min']
            ws_pool = []
            for j, ws in enumerate(self.h_ws_sizes): # [1, 2, 36]
                with tf.name_scope('holistic-conv-{}pool-{}'.format(pool, ws)):
                    # conv layers
                    filter_shape = [ws, self.embedding_size, 1, self.filter_size]
                    W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name='W')
                    conv_s1 = tf.nn.conv2d(sent, W, strides=[1, 1, 1, 1], padding='VALID', name='conv')
                    # pool layers
                    ...
                    #print(pool_out_expand.shape)
                    ws_pool.append(pool_out_expand)
            m_pool.append(ws_pool)

        return tf.concat(m_pool, 3) # shape of [3, ?, 300, 3]

    def cal_holistic(self, m1_pool, m2_pool):
        with tf.name_scope('cal_holistic'):
            split1_max, split1_mean, split1_min = [tf.squeeze(sp, [0]) for sp in tf.split(m1_pool, 3, 0)] # shape of [?, 300, 3]
    ...
    def inference_perspective(self, sent):
        m_pool = []
        for i, pool in enumerate(self.p_pool_type): # ['max', 'min']
            ws_pool = []
            for j, ws in enumerate(self.p_ws_sizes): # [1, 2]
                with tf.name_scope('perspective-conv-{}pool-{}'.format(pool, ws)):
                    ...
                    #print(conv_s1.shape) # [?, 36, 50, 300], [?, 35, 50, 300]
                    # pool layers
                    k_size = [1, self.sequence_length - ws + 1, 1, 1]
                    if pool == 'max':
                        pool_out = tf.nn.max_pool(conv_s1, ksize=k_size, strides=[1, 1, 1, 1],
                                                  padding='VALID', name='pool')
                    else:
                        pool_out = -tf.nn.max_pool(-conv_s1, k_size, strides=[1, 1, 1, 1],
                                                   padding='VALID', name='pool')
                    #print(pool_out.shape) # [?, 1, 50, 300]
                    pool_out = tf.transpose(tf.squeeze(pool_out, [1]), [0, 2, 1])
                    #print(pool_out.shape) # [?, 300, 50]
                    ws_pool.append(pool_out)
            m_pool.append(ws_pool)
        #print(tf.concat(m_pool, 3).shape) # [2, ?, 300, 100]
        return tf.concat(m_pool, 3)

    def cal_perspective(self, m1_pool, m2_pool):
        with tf.name_scope('cal_perspective'):
            split1_max, split1_min = [tf.squeeze(sp, [0]) for sp in tf.split(m1_pool, 2)] # shape of [?, 300, 100]
            split2_max, split2_min = [tf.squeeze(sp, [0]) for sp in tf.split(m2_pool, 2)]
            # calculate heah holistically
            cal_feah_max = tf.reduce_sum(tf.multiply(split1_max, split2_max, name="cal_feah_max"), axis=2) # shape of [?, 300]
            cal_feah_min = tf.reduce_sum(tf.multiply(split1_min, split2_min, name="cal_feah_min"), axis=2)
            feah_holistic = tf.concat([cal_feah_max, cal_feah_min], axis=1, name="feah_concat") # shape of [?, 600]

            # calculate heab holistically
            cal_feab_max = tf.reduce_sum(tf.multiply(split1_max, split2_max, name="cal_feab_max"), axis=1) # shape of [?, 100]
            cal_feab_min = tf.reduce_sum(tf.multiply(split1_min, split2_min, name="cal_feab_min"), axis=1)
            feab_holistic = tf.concat([cal_feab_max, cal_feab_min], axis=1, name="feab_concat") # shape of [?, 200]
    ...
    def add_output(self):
        ...
        with tf.name_scope('output'):
            W = tf.get_variable('W', shape=[self.temp_concat.shape[1], self.num_classes],
